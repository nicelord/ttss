...
public class MenuSetoranVM {
    ...
    public void initSetup() {
        ...
        this.listTTSS = (List<TTSS>) Ebean.find((Class) TTSS.class).where().eq("tipe", (Object) "masuk").setMaxRows(50).orderBy("wktTerima desc").findList();
        Long nilai = 0L;
    ...
    public void downloadXLS() {
        ...
        try {
            InputStream streamReport = JRLoader.getFileInputStream(Executions.getCurrent().getDesktop().getWebApp().getRealPath("/") + "/report/setoran.masuk.jasper");
            ...
            exporterXLS.setParameter((JRExporterParameter) JRXlsExporterParameter.IS_ONE_PAGE_PER_SHEET, true);
            exporterXLS.setParameter((JRExporterParameter) JRXlsExporterParameter.IS_DETECT_CELL_TYPE, true);
            exporterXLS.setParameter((JRExporterParameter) JRXlsExporterParameter.IS_WHITE_PAGE_BACKGROUND, true);
            exporterXLS.setParameter((JRExporterParameter) JRXlsExporterParameter.IS_REMOVE_EMPTY_SPACE_BETWEEN_ROWS, true);
            exporterXLS.setParameter((JRExporterParameter) JRXlsExporterParameter.IS_REMOVE_EMPTY_SPACE_BETWEEN_COLUMNS, true);
    ...
    public void hapusSelectedTTSS() {
        for (final TTSS ttss1 : this.selectedTTSS) {
            try {
                ...
                Logger.getLogger(MenuSetoranVM.class.getName()).log(Level.SEVERE, null, e);
            }
        }
        this.refresh();
    }
