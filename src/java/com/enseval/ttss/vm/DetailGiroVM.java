...
public class DetailGiroVM {
    ...
    public void initSetup(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("gironya") Giro gironya) {
        ...
        try {
            txtCUstID.setValue(this.giro.getCustomer().getId().toString());
            txtCustName.setValue(Ebean.find(Customer.class, this.giro.getCustomer().getNama()).getNama());
    ...
    public void UpdateGiro() {
        ...
        if (!txtCUstID.getValue().isEmpty()) {
            this.giro.setCustomer(Ebean.find(Customer.class, Long.valueOf(txtCUstID.getValue())));
        }
